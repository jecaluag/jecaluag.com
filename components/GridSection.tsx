...
const GridSection = ({ title, size = 'md', items }: Props): JSX.Element => {
  return (
    <section className='mb-14'>
      ...
      <div>
        {items.map(item => (
          <div key={item.title} className={`grid-item ${size === 'md' ? 'mb-10' : 'mb-4'}`}>
            ...
            <div className='col-span-4 sm:col-span-3'>
              {has(item, 'url') ? 
                (
                  <a href={item?.url} target="_blank" rel="noreferrer" className='text-white flex items-center hover:text-neutral-200'>
                    {item.title}
              ...
              {has(item, 'description') && (
                Array.isArray(item?.description) ? (
                  item?.description.map(desc => <p key={desc} className='my-2'>{desc}</p>)
